...
function TaskDrawer({ opened, onClose, task, priorities, viewingDate }) {
    ...
    const empty = { description: '', notes: '', priority_id: null, scheduled_for: viewingDate, scheduled_time: '' };
                    ...
                    <Group grow align='flex-end'>
                        ...
                        <Box>
                            ...
                            <TimeInput
                                ...
                                rightSection={
                                    form.scheduled_time
                                        ? <IconX size={14} style={{ cursor: 'pointer' }}
...
function TaskRow({ task, onEdit, isOverdue }) {
    ...
    const isCompleted = optimisticCompleted !== null ? optimisticCompleted : task.completed_at !== null;
    ...
    function handleToggle() {
        ...
        router.post(route('personal-tasks.toggle', task.id), {}, {
            ...
            onError:  () => { setOptimisticCompleted(null); setToggling(false); },
            onFinish: () => { setToggling(false); setOptimisticCompleted(null); },
        });
    }
    ...
    if (isCompleted && task.completed_at) {
        ...
        dateLabel = task.scheduled_time
            ? `Hoy · ${task.scheduled_time.slice(0, 5)}`
            : 'Hoy';
    }
        ...
        <>
            <div className={`${classes.taskRow} ${isCompleted ? classes.taskRowCompleted : ''}`}>
                ...
                <div className={classes.taskCheck}>
                    {toggling
                        ? <Loader size={14} />
                        : <Checkbox checked={isCompleted} onChange={handleToggle} radius='sm' size='sm' />
                    }
                </div>
                ...
                <div className={classes.taskBody}>
                    <Text
                        size='sm' fw={isCompleted ? 400 : 500}
                    ...
                    {task.notes && (
                        <Text size='xs' c='dimmed' lineClamp={1} className={classes.taskNotes}>
                            {task.notes}
                        </Text>
                    )}
                </div>
                ...
                {priorityColor && (
                    <div className={classes.taskMeta}>
                        <ColorSwatch color={priorityColor} size={8} />
                        <Text size='xs' c='dimmed'>{priorityLabel}</Text>
                    </div>
                )}

                {/* Fecha / hora */}
                <div className={`${classes.taskMeta} ${classes.taskDate}`}>
                    <DateIcon size={13} className={classes.metaIcon} />
                ...
                <Menu shadow='md' position='bottom-end' withinPortal>
                    <Menu.Target>
                        <ActionIcon variant='subtle' color='gray' size='sm' className={classes.menuTrigger}>
...
const PersonalTasksIndex = () => {
    ...
    function navigateTo(dateStr) {
        const newParams = { ...params, date: dateStr };
        if (dateStr === today) delete newParams.date; // hoy = URL limpia
        reloadWithQuery(newParams, true);
    }
                ...
                {!hasAnything ? (
                    <Center mih={200}>
                        <Stack align='center' gap='xs'>
                            <IconCalendarCheck size={38} strokeWidth={1.2} color='var(--mantine-color-dimmed)' />
                            <Text c='dimmed' size='sm' ta='center'>
                                No hay tareas para este día.<br />
                            ...
                            <div className={classes.section}>
                                ...
                                <div className={classes.sectionBody}>
                                    {overdueTasks.map(task => (
                                        <TaskRow key={task.id} task={task} onEdit={handleEdit} isOverdue />
                                    ))}
                                </div>
                            </div>
                        )}
